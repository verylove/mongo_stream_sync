...
class ElasticManager {
  ...
  transformDoc (collName, esReadyDoc) {
    ...
    if (transformFunc) {
      return this._transformFunc(esReadyDoc, transformFunc)
  ...
  /**
   * function to remove unnecessary field from esReadyDoc
   * @param {json} esReadyDoc document from mongoDB, to be inserted to elasticsearch
   * @param {dictionary} functions define necessary/unnecessary field
   * @returns {json} return document in wich unnecessary fields are been removed
   */
  _transformFunc (esReadyDoc, transformFunc) {
    const necessary = transformFunc.necessary ? transformFunc.necessary : []
    const unnecessary = transformFunc.unnecessary ? transformFunc.unnecessary : []

    if (necessary.length === 0 && unnecessary.length === 0) {
      return esReadyDoc
    }
    // skip necessary field check for now

    // process unnecessary field
    for (const field of unnecessary) {
      if (esReadyDoc[field]) {
        delete esReadyDoc[field]
      }
    }
